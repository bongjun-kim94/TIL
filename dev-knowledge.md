...
- 컴퓨터끼리 네트워크 통신을 위해 약속하는 통신규약
- 웹을 이용할 땐, HTTP 프로토콜을 이용
- 요즘은 HTTP 프로토콜에서 보안을 더한 HTTPS 프로토콜이 많이 사용됨
- 이메일을 통신할떄 사용하는 SMTP, 원격 통신에 사용되는 SSH 등이 있다.
...
- 요청과 응답 모두 `요청 -> 헤더 -> 공백 -> 바디` 구조로 이루어져 있다.
...
- HTTP 통신으로 서버에게 정보를 요청하기 위해선 몇가지 규칙을 지켜야 함
  - 어떤 서버에게 요청할 것인지 `URL`이 있어야 한다. (요청 라인)
  - 어떤 방식으로 통신을 할 것인지 `Method` 가 있어야 한다 (요청 라인)

### HTTP 응답

- 서버에서는 HTTP 요청을 받으면 응답을 준다.
- HTTP 응답의 규칙으로
  - 응답이 어떤 상태인지를 나타내는 Status Code 가 있어야 한다.
  - 요청결과를 Body에 담아야한다.
- Status Code는 200~500번 까지 있고, 서버의 응답에 대한 상태를 표현
  - `200`번 대는 정상적으로 응답
  - `400`번 대는 클라이언트가 요청을 잘 못 했을때
  - `500`번 대는 서버에서 에러가 발생
